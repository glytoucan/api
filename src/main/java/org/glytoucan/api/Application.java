...
//				.paths(PathSelectors.any()).build().pathMapping("/")
				.paths(allPaths()).build().pathMapping("/")
	...
	private Predicate<String> allPaths() {
		return or(regex("/glycan/register"), regex("/status"));
	}

	private ApiInfo apiInfo() {
		return new ApiInfoBuilder().title("Glytoucan REST API")
				.description(
						"This API can be used as a REST api server or in combination with " + "the glytoucan client.  "
